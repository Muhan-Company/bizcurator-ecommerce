...
export default function Four({ formValues4 }: { formValues4: FormValues4 }) {
  const { title, description, desiredEstimateDate } = formValues4;
  ...
  return (
    <div className="mt-12 gap-y-1.5 flex flex-col">
      ...
      <section className="bg-gray-100 flex items-center px-4 space-x-2  rounded-lg h-[50px]">
        ...
        <span className="font-normal text-main text-body-xs">{desiredEstimateDate}</span>
      </section>
    </div>
  );
}

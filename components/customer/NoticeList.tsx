      ...
      <div className="relative">
        ...
        {item?.map((item, index) => (
          <div key={index}>
            ...
            {selectIndex === index && (
              <div className="bg-[#fafafa] break-words py-9 px-5">
                <div>
                  <h3 className="hidden md:block md:ml-8 md:mb-6">- {item.title}</h3>
                  <span className="text-xs md:ml-5">{item.content}</span>
                  <span className="text-xs inline-block py-5">감사합니다.</span>
                </div>
                <button
